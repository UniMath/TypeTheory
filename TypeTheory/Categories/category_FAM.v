...
Proof.
  intro H.
  apply (gradth _ (pr1 (inv_from_z_iso (f,, H)))).
  - intro x. 
    apply (toforallpaths (maponpaths pr1 (z_iso_inv_after_z_iso (f,, H)))).
  - intro x.
    apply (toforallpaths (maponpaths pr1 (z_iso_after_z_iso_inv (f,, H)))).
Defined.
...
Proof.
  ...
  set (x  := _,,H': z_iso (A ₂ (finv b)) (B ₂ (pr1 f (finv b)))).
...
Lemma FAM_is_iso_from_is_z_iso (A B : FAM C) (f : A --> B) : is_z_isomorphism f → FAM_is_iso f.
Proof.
  intro f_iso.
  split.
  - apply isweq_from_is_iso. assumption.
  - set (g := z_iso_inv_from_z_iso (f,, f_iso) : B --> A).
...
Proof.
  apply weqimplimpl.
  - apply FAM_is_iso_from_is_z_iso.
