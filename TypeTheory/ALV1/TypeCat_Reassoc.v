...
Context
  ...
  (T_reind : T_ty -> UU)
  (T_q : Π ty, T_ext ty -> T_reind ty -> UU)
  (T_dpr_q : Π ty ext reind, (T_dpr ty ext) -> (T_q ty ext reind) -> UU)
  (T_reind_pb : Π ty ext reind dpr q, (T_dpr_q ty ext reind dpr q) -> UU)
  (T_set : T_ty -> UU)
  (T_reind_id : Π ty, T_reind ty -> UU)
  (T_q_id : Π ty ext reind, (T_q ty ext reind) -> (T_reind_id ty reind) -> UU)
  (T_reind_comp : Π ty, T_reind ty -> UU)
  (T_q_comp : Π ty ext reind, (T_q ty ext reind) -> (T_reind_comp ty reind) -> UU).

Arguments T_dpr [_] _.
Arguments T_q [_] _ _.
Arguments T_dpr_q [_ _ _] _ _.
Arguments T_reind_pb [_ _ _ _ _] _.
Arguments T_reind_id [_] _.
Arguments T_q_id [_ _ _] _ _.
Arguments T_reind_comp [_] _.
Arguments T_q_comp [_ _ _] _ _.
...
:= Σ dpr q dpr_q, @T_reind_pb ty ext reind dpr q dpr_q.

Definition struct_total := Σ S1, struct2 S1.

Definition is_split (S : struct_total)
  (ty := pr1 (pr1 S)) (reind := pr2 (pr2 (pr1 S))) (q := pr1 (pr2 (pr2 S)))
:= T_set ty
  × (Σ reind_id, T_q_id q reind_id)
  × (Σ reind_comp, T_q_comp q reind_comp).
...
Definition gen_q_mor_data (X : obj_ext_struct)
  ...
  (reind := pr2 (pr1 (pr1 X))) 
:= Σ (q : T_q ext reind) (dpr_q : T_dpr_q dpr q), T_reind_pb dpr_q.

Definition gen_q_mor_axs {X : obj_ext_struct} (Q : gen_q_mor_data X)
  (q := pr1 Q) (A := pr2 (pr1 X)) (reind_id := pr1 A) (reind_comp := pr2 A)
:= T_q_id q reind_id × T_q_comp q reind_comp.

Definition gen_q_mor_struct (X : obj_ext_struct)
:= Σ (Q : gen_q_mor_data X), gen_q_mor_axs Q.
...
Definition T_q
  := (λ ty (ext : T_ext ty) (reind : T_reind ty), 
       Π (Γ:CC) (A : ty Γ) Γ' (f : Γ' --> Γ),
         (ext Γ' (reind _ A _ f)) --> (ext Γ A)).
Definition T_dpr_q
  := (λ ty ext reind (dpr : T_dpr ty ext) (q : T_q ty ext reind),
       Π Γ (A : ty Γ) Γ' (f : Γ' --> Γ),
         q _ A _ f ;; dpr Γ A = dpr Γ' (reind _ A _ f) ;; f).
Definition T_reind_pb
  := (λ ty ext reind dpr q (dpr_q : T_dpr_q ty ext reind dpr q),
       Π Γ (A : ty Γ) Γ' (f : Γ' --> Γ),
...
Definition T_q_id
  := (λ ty ext reind (q : T_q ty ext reind) (reind_id : T_reind_id ty reind),
       Π Γ A, q _ A _ (identity Γ)
...
Definition T_q_comp
  := (λ ty ext reind (q : T_q ty ext reind) (reind_comp : T_reind_comp ty reind),
       Π Γ A Γ' (f : Γ' --> Γ) Γ'' (g : Γ'' --> Γ'),
         q _ A _ (g ;; f)
         = idtoiso (maponpaths (ext Γ'') (reind_comp _ A _ f _ g))
               ;; q _ (reind _ A _ f) _ g
               ;; q _ A _ f).

Definition eq_standalone_structural
  : split_type_struct CC
    = split_struct
        T_ty T_ext T_dpr T_reind T_q T_dpr_q T_reind_pb
...
(** Alternate construction of [weq_standalone_structural], retained in case they have better computational behaviour: *)
...
- try condensing q, dpr_q, pb into a single item; see how that improves compilation time. *)
