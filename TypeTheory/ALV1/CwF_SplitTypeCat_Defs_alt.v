...
Definition term_fun_structure_data : UU
  := Σ Tm : preShv C, 
        (Tm --> TY X)
        × (Π (Γ : C) (A : Ty X Γ), ( (Tm : functor _ _ ) (Γ ◂ A) : hSet)).

Definition TM (Y : term_fun_structure_data) : preShv C := pr1 Y.
(* Notation "'Tm' Y" := (TM Y : functor _ _ )(at level 3). *)
Definition pp Y : TM Y --> TY X := pr1 (pr2 Y).
Definition v (Y : term_fun_structure_data) {Γ : C} (A : Ty X Γ) : ((TM Y : functor _ _) (Γ ◂ A) : hSet)
  := pr2 (pr2 Y) Γ A.

Definition Q Y {Γ : C} (A : Ty X Γ) : _ --> TM Y := yy (hsC := homset_property C) (v Y A).
...
Lemma cwf_square_comm (Y : term_fun_structure_data) (Γ : C) (A : Ty X Γ )
  (H : (pp Y : nat_trans _ _ )  _ (v Y A) = (A [π A ])) : 
  #Yo (π _ ) ;; yy A = yy (v Y A) ;; pp Y.
Proof.
  apply pathsinv0.
...
Definition term_fun_structure_axioms (Y : term_fun_structure_data) :=
  Π Γ (A : Ty X Γ), 
        Σ (e : (pp Y : nat_trans _ _ )  _ (v Y A) = (A [π A ])), 
        isPullback _ _ _ _ (cwf_square_comm Y _ _ e).
...
:= cwf_square_comm Y _ _ (pr1 (pr2 Y _ _ )).
