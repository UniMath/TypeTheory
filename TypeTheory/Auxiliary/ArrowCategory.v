...
Definition arrow_category_is_z_iso {C : category}
           {abf cdg : arrow_category C}
...
Lemma isaprop_arrow_category_is_z_iso {C : category}
           {abf cdg : arrow_category C}
           (a_to_c : C ⟦ pr1 (pr1 abf), pr1 (pr1 cdg) ⟧)
           (b_to_d : C ⟦ dirprod_pr2 (pr1 abf), dirprod_pr2 (pr1 cdg) ⟧)
  : isaprop (arrow_category_is_z_iso a_to_c b_to_d).
Proof.
  use isapropdirprod.
...
Definition arrow_category_is_z_iso_to_is_z_iso {C : category}
           {abf cdg : arrow_category C}
           (a_to_c : C ⟦ pr1 (pr1 abf), pr1 (pr1 cdg) ⟧)
           (b_to_d : C ⟦ dirprod_pr2 (pr1 abf), dirprod_pr2 (pr1 cdg) ⟧)
  : arrow_category_is_z_iso a_to_c b_to_d
    → ∑ (p : pr2 abf ;; b_to_d = a_to_c ;; pr2 cdg)
    , is_z_isomorphism (((a_to_c,, b_to_d),,p) : arrow_category C ⟦ abf, cdg ⟧).
Proof.
  intros h.
  set (is_z_iso_a_to_c := pr1 (dirprod_pr2 h)).
  set (is_z_iso_b_to_d := pr1 h).
  set (z_iso_a_to_c := (a_to_c,,is_z_iso_a_to_c)).
  set (z_iso_b_to_d := (b_to_d,,is_z_iso_b_to_d)).
  ...
  - unfold is_z_isomorphism.
    ...
    + use tpair.
      * use make_dirprod.
        -- apply (inv_from_z_iso z_iso_a_to_c).
        -- apply (inv_from_z_iso z_iso_b_to_d).
      * simpl.
        apply pathsinv0.
        etrans. apply pathsinv0, id_right.
        etrans. apply maponpaths, pathsinv0, (z_iso_inv_after_z_iso z_iso_b_to_d).
        etrans. apply assoc'.
    ...
    + use make_dirprod.
      * use total2_paths_f.
        -- use dirprod_paths.
           ++ apply (z_iso_inv_after_z_iso z_iso_a_to_c).
           ++ apply (z_iso_inv_after_z_iso z_iso_b_to_d).
        -- apply homset_property.
      * use total2_paths_f.
        -- use dirprod_paths.
           ++ apply (z_iso_after_z_iso_inv z_iso_a_to_c).
           ++ apply (z_iso_after_z_iso_inv z_iso_b_to_d).
        -- apply homset_property.
Defined.

Definition is_z_iso_to_arrow_category_is_z_iso {C : category}
           {abf cdg : arrow_category C}
           (a_to_c : C ⟦ pr1 (pr1 abf), pr1 (pr1 cdg) ⟧)
           (b_to_d : C ⟦ dirprod_pr2 (pr1 abf), dirprod_pr2 (pr1 cdg) ⟧)
  : (∑ (p : pr2 abf ;; b_to_d = a_to_c ;; pr2 cdg)
...
Definition arrow_category_weq_is_z_iso {C : category}
           {abf cdg : arrow_category C}
           (a_to_c : C ⟦ pr1 (pr1 abf), pr1 (pr1 cdg) ⟧)
           (b_to_d : C ⟦ dirprod_pr2 (pr1 abf), dirprod_pr2 (pr1 cdg) ⟧)
  : arrow_category_is_z_iso a_to_c b_to_d
...
Proof.
  use weq_iso.
  - apply arrow_category_is_z_iso_to_is_z_iso.
  - apply is_z_iso_to_arrow_category_is_z_iso.
  - intros h. apply isaprop_arrow_category_is_z_iso.
  - intros h.
    use total2_paths_f.
    + apply homset_property.
    + apply isaprop_is_z_isomorphism.
...
Definition arrow_category_id_weq_z_iso {C : category}
...
Proof.
  ...
  - eapply weqcomp. apply weq1.
    ...
    + intros a_to_c.
      ...
      * apply idweq.
      * intros b_to_d. simpl.
        apply arrow_category_weq_is_z_iso.
Defined. 
...
Proof.
  intros abf cdg.
  use isweqhomot.
  + apply arrow_category_id_weq_z_iso.
    apply C_univ.
  + intros p. induction p.
    ...
    * apply idpath.
      * apply idpath.
  + apply (pr2 (arrow_category_id_weq_z_iso C_univ _ _)).
Defined.
